...
#define GETBYTE(x, y) (word32)((byte)(((x) >> (8 * (y))) & 0xFF))
...
static void wc_AesEncrypt(Aes* aes, const byte* inBlock, byte* outBlock)
{
     ...
     */
    DBC_ByteToWord32Array(&s0, inBlock                        , DBC_SIZEOF_WORD32);
    DBC_ByteToWord32Array(&s1, inBlock + DBC_SIZEOF_WORD32    , DBC_SIZEOF_WORD32);
    DBC_ByteToWord32Array(&s2, inBlock + 2 * DBC_SIZEOF_WORD32, DBC_SIZEOF_WORD32);
    DBC_ByteToWord32Array(&s3, inBlock + 3 * DBC_SIZEOF_WORD32, DBC_SIZEOF_WORD32);
    ...
    DBC_Word32ToByteArray(outBlock                        , &s0, DBC_SIZEOF_WORD32);
    DBC_Word32ToByteArray(outBlock + DBC_SIZEOF_WORD32    , &s1, DBC_SIZEOF_WORD32);
    DBC_Word32ToByteArray(outBlock + 2 * DBC_SIZEOF_WORD32, &s2, DBC_SIZEOF_WORD32);
    DBC_Word32ToByteArray(outBlock + 3 * DBC_SIZEOF_WORD32, &s3, DBC_SIZEOF_WORD32);
...
/* wc_AesSetKey */
                ...
                const byte* iv, int dir)
    {
        ...
        DBC_ByteToWord32Array(rk, userKey, keylen);
...
int wc_AesSetIV(Aes* aes, const byte* iv)
{
    ...
    if (iv)
        DBC_ByteToWord32Array(aes->reg, iv, AES_BLOCK_SIZE);
    else
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        while (blocks--) {
            DBC_XorByteIntoWord32Array(aes->reg, in, AES_BLOCK_SIZE);
            DBC_Word32ToByteArray(reg, aes->reg, AES_BLOCK_SIZE);
            wc_AesEncrypt(aes, reg, reg);
            DBC_ByteToWord32Array(aes->reg, reg, AES_BLOCK_SIZE);
            DBC_Word32ToByteArray(out, aes->reg, AES_BLOCK_SIZE);
    ...
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        while (blocks--) {
            DBC_ByteToWord32Array(aes->tmp, in, AES_BLOCK_SIZE);
